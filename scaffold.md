...
- Formatting-only changes (handled by hooks)
```
...
Pick the right hooks for the project's language/framework:
...
```json
{
  "hooks": {
    "PostEditTool": [
      { "command": "ruff format $CLAUDE_FILE_PATHS 2>/dev/null; ruff check --fix $CLAUDE_FILE_PATHS 2>/dev/null; true" }
    ]
  }
}
```
...
If no standard formatter exists for the language, skip PostEditTool hooks entirely. Do NOT put formatting rules in CLAUDE.md as a substitute.

---

## Settings Template

The `.claude/settings.json` file combines hooks, security deny patterns, and optional LSP configuration. Build the settings file by merging the relevant sections:

### Security Defaults (Always Include)
...
### Combined Example (TypeScript + Security + Safety Hook)
...
Create in `.claude/agents/` only for projects with clear, repeatable workflows.

### Test Runner (Common Default)
...
model: sonnet
---
Run the project's test suite. Report:
- Total tests: passed, failed, skipped
- Full output of any failures
- Suggested fixes for failures
```

Only scaffold agents when the user confirms the project needs them.

---

## Commands Template

Create in `.claude/commands/` only for frequently repeated workflows.

### Catchup Command

```markdown
---
description: Review all changes on current branch vs main
---
1. Run `git diff --name-only main...HEAD` to list changed files
2. Read each changed file
3. Summarize: what changed, why, any concerns
```

Only scaffold commands when the user confirms they want them.
